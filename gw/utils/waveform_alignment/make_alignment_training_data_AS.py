...
def make_dtphis(injection_parameters,
    ...
    if mode in ['plus', 'cross']:
        ...
        for i,approx in enumerate(approximant_list):
            ...
            if i==0:
                h_list.append(h['plus'])
                h_list.append(h['cross'])
            else:
                #h = get_shifted_h2_zeropad(h_list[0],h,det)
                deltat_plus,deltaphi_plus = get_dtdphi_withift_zeropad(h_list[0],h['plus'],det)
                deltat_cross,deltaphi_cross = get_dtdphi_withift_zeropad(h_list[1],h['cross'],det)
        return deltat_plus,deltaphi_plus, deltat_cross,deltaphi_cross
...
# nohup python make_alignment_training_data_AS.py ASHM 40 >nohup_ASHM40.out &
# nohup python make_alignment_training_data_AS.py AS 40 >nohup_AS40.out &
if __name__ == '__main__':
    ...
    fref_list = np.random.uniform(low=5, high=200, size=N)
    ...
    parameter_name_list = ['chirp_mass','mass_ratio','a_1','a_2','theta_jn']
    for paraname in parameter_name_list:
        data_dict['source_parameters'][paraname] = []
    data_dict['source_parameters']['f_ref'] = fref_list

    data_dict['outputs'] =  dict()
    for outputname in ['dt_plus', 'dphi_plus','dt_cross', 'dphi_cross']:
    ...
    for waveform_index in range(N):
        ...
        deltat_plus,deltaphi_plus, deltat_cross,deltaphi_cross = make_dtphis(injection_para,
            duration, f_lower, sampling_frequency,
            approximant_list=approximant_list, mode='both', f_ref=fref_list[waveform_index])
        
        data_dict['outputs']['dt_plus'].append(deltat_plus)
        data_dict['outputs']['dphi_plus'].append(deltaphi_plus)
        data_dict['outputs']['dt_cross'].append(deltat_cross)
        data_dict['outputs']['dphi_cross'].append(deltaphi_cross)
    ...
    save_folder = ''
    h5filename = save_folder + f'dtdphi_5D_q10a99M{Mtot}_IMREOB_{phy}.h5'
    # 1: 4s, 4096Hz
    # 2: 16s, 4096Hz, //3
    # 3: 32s, 8192Hz, //10
